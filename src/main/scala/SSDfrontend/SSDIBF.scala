...
class SSDIBF extends NutCoreModule with HasInstrType with HasIBUFConst{
  ...
  val ringBufferAllowin = (0 to 7).map(i => !validRingMeta(ringBufferHead+i.U)).foldRight(true.B)((sum,i)=>sum&i)

  //ibuf decode
  val instr = io.in.bits.instr
  val instrVec = Wire(Vec(8, UInt(16.W)))
  val isRVC = Wire(Vec(8, Bool()))
  ...
  instrVec := instr.asTypeOf(Vec(8, UInt(16.W)))
  (0 to 7).map(i => isRVC(i.U) := instrVec(i.U)(1,0) =/= "b11".U)

  //ibuf enqueue
  //if valid & ringBufferAllowin, enqueue
  val needEnqueue = Wire(Vec(8, Bool()))
  needEnqueue(0) := instValid(0)
  needEnqueue(1) := instValid(1) && !(brIdx(0) && isRVC(0))
  needEnqueue(2) := instValid(2) && !brIdx(0) && !(brIdx(1) && isRVC(1))
  needEnqueue(3) := instValid(3) && !brIdx(1,0) && !(brIdx(2) && isRVC(2))
  needEnqueue(4) := instValid(4) && !brIdx(2,0) && !(brIdx(3) && isRVC(3))
  needEnqueue(5) := instValid(5) && !brIdx(3,0) && !(brIdx(4) && isRVC(4))
  needEnqueue(6) := instValid(6) && !brIdx(4,0) && !(brIdx(5) && isRVC(5))
  needEnqueue(7) := instValid(7) && !brIdx(5,0) && !(brIdx(6) && isRVC(6))
  ...
  val enqueueSize = needEnqueue(0).asUInt()+&needEnqueue(1).asUInt()+&needEnqueue(2).asUInt()+&needEnqueue(3).asUInt() +&
    needEnqueue(4).asUInt() +&needEnqueue(5).asUInt() +&needEnqueue(6).asUInt() +&needEnqueue(7).asUInt()// count(true) in needEnqueue
  val shiftSize = Mux(needEnqueue(0), 0.U, Mux(needEnqueue(1), 1.U, Mux(needEnqueue(2), 2.U, Mux(needEnqueue(3), 3.U, Mux(needEnqueue(4), 4.U,
    Mux(needEnqueue(5), 5.U, Mux(needEnqueue(6), 6.U, 7.U))))))) // count 0 in low addr in needEnqueue
  val enqueueFire = (0 to 7).map(i => enqueueSize >= (i+1).U)
  ...
  def ibufWrite(targetSlot: Int, shiftSize: UInt){
    ...
    pcRingMeta(targetSlot.U + ringBufferHead) := Cat(io.in.bits.pc(VAddrBits-1, 4), shiftSize + targetSlot.U, 0.U(1.W))
  ...
  when(ibufWen){
    ...
    when(enqueueFire(4)){ibufWrite(4, shiftSize)}
    when(enqueueFire(5)){ibufWrite(5, shiftSize)}
    when(enqueueFire(6)){ibufWrite(6, shiftSize)}
    when(enqueueFire(7)){ibufWrite(7, shiftSize)}
